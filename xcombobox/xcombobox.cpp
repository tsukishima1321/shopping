...
void XComboBox::xAddItem(const QString &str, bool bChecked /*= false*/, const QVariant &userData /*= QVariant()*/) {
...
void XComboBox::xAddItems(const QList<ItemInfo> &lstItemInfo) {
    for (auto a : lstItemInfo) {
        xAddItem(a.str, a.bChecked, a.userData);
    }
}

void XComboBox::xAddItems(const QMap<QString, bool> &mapStrChk) {
    for (auto it = mapStrChk.begin(); it != mapStrChk.end(); ++it) {
        xAddItem(it.key(), it.value());
    }
}

void XComboBox::xAddItems(const QList<QString> &lstStr) {
    for (auto a : lstStr) {
        xAddItem(a, false);
    }
}
