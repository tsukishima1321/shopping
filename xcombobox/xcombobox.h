...
public:
    ...
    // 添加item
    void xAddItem(const QString &str, bool bChecked = false, const QVariant &userData = QVariant());
    void xAddItems(const QList<ItemInfo> &lstItemInfo);
    void xAddItems(const QMap<QString, bool> &mapStrChk);
    void xAddItems(const QList<QString> &lstStr);
    // 删除item
